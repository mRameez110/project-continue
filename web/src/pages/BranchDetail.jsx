...
const BranchDetail = () => {
	...
	const [updatedBranchData, setUpdatedBranchData] = useState(null);
	const [pharmacists, setPharmacists] = useState([]);
	const [selectedPharmacists, setSelectedPharmacists] = useState([]); // Track selected pharmacists
	...
	useEffect(() => {
		const fetchPharmacists = async () => {
			try {
				...
				// Filter pharmacists who are not assigned to any branch
				const pharmacistsWithoutBranch = response.data.pharmacists.filter(
					(pharmacist) => !pharmacist.pharmacyBranch
				);
				setPharmacists(pharmacistsWithoutBranch.reverse());
	...
	// Handle input change for editing branch details
	const handleInputChange = (field, value) => {
		setUpdatedBranchData({
			...updatedBranchData,
			[field]: value,
	...
	// Handle submitting edited branch data
	const handleEdit = async () => {
		try {
			const { _id, ...dataToUpdate } = updatedBranchData;
			// Send selected pharmacists (or an empty array if none are selected)
